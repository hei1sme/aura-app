...
<script lang="ts">
  ...
  $: content = $currentBreak
    ? breakContent[$currentBreak.break_type] || breakContent.micro
    : breakContent.micro;
  ...
  onMount(async () => {
    ...
    unlistenShowBreak = await listen("show-break", (event) => {
      ...
      const breakData = event.payload as {
...
<div class="overlay-card" style="--theme-color: {themeColor}">
  {#if $currentBreak}
    ...
    {#if state === "action"}
      <div class="card-content animate-fade-in">
        <div
          ...
          style="--progress: {($breakCountdown / duration) * 100}%"
        >
          <span class="countdown-text" style="color: {themeColor}"
            >{formatTime($breakCountdown)}</span
          >
        </div>
        <p class="instruction">{content.instruction}</p>
    ...
    {#if state === "hydration-select"}
      <div class="card-content animate-fade-in">
        <div class="emoji">💧</div>
        <h2 class="title" style="color: {themeColor}">
    ...
    {#if state === "complete"}
      <div class="card-content animate-fade-in">
        ...
        <button class="btn-start btn-success" on:click={handleComplete}
          >Done</button
        >
      </div>
    {/if}
