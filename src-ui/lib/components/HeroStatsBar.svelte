...
<script lang="ts">
  ...
  $: wellnessScore = Math.round(
    ...
    ((1 - breakProgress) * 100 * 0.6)
  );
  
  $: wellnessColor = wellnessScore >= 70 ? 'var(--aura-eye-care)' : 
...
<div class="hero-stats-bar">
  ...
  <!-- Hydration -->
  <div class="stat-pill">
    <span class="stat-value text-lg font-light text-info">{$hydrationPercent}%</span>
    <span class="stat-label">💧 Hydration</span>
  </div>
  
  <!-- Breaks Today -->
  <div class="stat-pill">
    <span class="stat-value text-lg font-light text-success">{todayBreakCount}</span>
  ...
  <div class="stat-pill wellness-pill flex-1">
    <div class="wellness-bar-container">
      <div class="flex justify-between items-center mb-1">
        ...
        <span class="text-sm font-medium" style="color: {wellnessColor}">{wellnessScore}%</span>
      </div>
